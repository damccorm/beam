...
func (f *formatFn) ProcessElement(s state.Provider, w string, c int) string {
	seen, err := f.State1.Contains(s, w)
	if err != nil {
		panic(err)
	}
	...
	err = f.State1.Add(s, fmt.Sprintf("%v%v", w, rand.Intn(100)))
	if err != nil {
		panic(err)
	}
	...
	if len(keys) > 5 {
		return fmt.Sprintf("%v: %v. Total num keys: %v. First 5 keys: %v", w, seen, len(keys), keys[:5])
	}

	return fmt.Sprintf("%v: %v. Total num keys: %v. With keys: %v", w, seen, len(keys), keys)
}
...
func main() {
	...
	formatted := beam.ParDo(s, &formatFn{State1: state.MakeSetState[string]("key1")}, counted)
